...
public class UAP_AccessibilityManager : MonoBehaviour
{
	...
	public static float PluginVersionAsFloat = 1.06f;
    ...
    float m_LongPressCounter = 0f;
    bool m_StartLongPressCounter = false;
    ...
    private OnTapEvent m_OnDoubleTapHandler = null;
    private OnTapEvent m_OnLongPressHandler = null;
    ...
    private OnTapEvent m_OnOneFingerSwipeRightHandler = null;
    private OnTapEvent m_OnOneFingerSwipeLeftHandler = null;
    private OnTapEvent m_OnOneFingerSwipeUpHandler = null;
    private OnTapEvent m_OnOneFingerSwipeDownHandler = null;
    ...
    private OnTapEvent m_OnThreeFingerSwipeUpHandler = null;
    private OnTapEvent m_OnThreeFingerSwipeDownHandler = null;
	...
	//////////////////////////////////////////////////////////////////////////
    static void ReadItem(AccessibleUIGroupRoot.Accessible_UIElement element, bool quickOnly = false, bool toggleReturn = false)
	{
		Initialize();
        ...
        // Injected call to the Base Class method for connecting PlayMaker to UAC 
        if (!toggleReturn)
        {
            //element.m_Object.CallPlayMakerHighlighted();
        }
...
*/
			...
			if (m_CurrentItem != null)
			{
				...
				if (m_PreviousItem != m_CurrentItem)
				{
					...
					if (isEnabled)
					{
                        if (m_PreviousItem != null && m_PreviousItem.m_Object != null)
                        {
                            m_PreviousItem.m_Object.HoverHighlight(false,true);
                        }
                ...
                //m_PreviousItem.m_Object.CallPlayMakerUnSelected();
                return;
			}
		}        
    }
    ...
    //////////////////////////////////////////////////////////////////////////
    /// <summary>
    /// TETRAGON : Adding a long press function for reading current menu
    /// </summary>
    private void CheckForLongPress()
    {
        if (Input.GetMouseButtonDown(0))
        {
            m_StartLongPressCounter = true;
        }
        if (Input.GetMouseButtonUp(0))
        {
            m_LongPressCounter = 0f;
            m_StartLongPressCounter = false;
        }
        if (m_StartLongPressCounter)
        {
            if (m_LongPressCounter > 3f)
            {
                if (m_OnLongPressHandler !=null)
                {
                    m_OnLongPressHandler();
                    m_LongPressCounter = 0f;
                    m_StartLongPressCounter = false;
                }
                else
                {
                    ...
                    Debug.Log("Reading Container Name");
            ...
            m_LongPressCounter += Time.deltaTime;
    ...
    public bool AudioQueueIsEmpty()
    {
        
        return instance.m_AudioQueue.m_IsSpeaking;
    }
		...
		if (m_DoubleTapFoundThisFrame)
		{
			...
			else
			{
                if (m_CurrentItem.m_Object.IsInteractable())
                {
                    ...
                    if (m_OnDoubleTapHandler != null)
                    {
                        m_OnDoubleTapHandler();
                    }
                    else
                    {
                        InteractWithElement(m_CurrentItem);
	...
	void InteractWithElement(AccessibleUIGroupRoot.Accessible_UIElement item)
	{
		...
		switch (item.m_Type)
		{
			...
			case AccessibleUIGroupRoot.EUIElement.EToggle:
				...
				ReadItem(m_CurrentItem, false, true);
				break;
			case AccessibleUIGroupRoot.EUIElement.ESlider:
	...
	public void OnSwipe(ESDirection dir, int fingerCount)
	{
		...
		{
            // Tetragon: Here is where the 1 finger swipes get handle!!
            if (fingerCount == 1)
			{
                ...
                if (HandleUI())
				{
					...
					else
					{
						...
						{
                            if (m_OnOneFingerSwipeRightHandler != null)
                            {

                                m_OnOneFingerSwipeRightHandler();
                            }
                            else
                            {
                                Debug.Log("CALL IS NULL");
                                InteractWithElement(m_CurrentItem);

                                // You can put a default method call
                            }
                            //IncrementUIElement();
                        }
						else if (dir == ESDirection.ELeft)
						{
                            if (m_OnOneFingerSwipeLeftHandler != null)
                            {
                                m_OnOneFingerSwipeLeftHandler();
                            }
                            else
                            {
                                Debug.Log("CALL IS NULL");
						...
						{
                            if (m_OnOneFingerSwipeDownHandler != null)
                            {
                                m_OnOneFingerSwipeDownHandler();
                            }
                            else
                            {
                                IncrementUIElement();
						...
						{
                            if (m_OnOneFingerSwipeUpHandler != null)
                            {
                                m_OnOneFingerSwipeUpHandler();
                            }
                            else
                            {
                                DecrementUIElement();
            ...
            // Tetragon: Here is where the 2 finger swipes get handle!!
            ...
            // Tetragon: Here is where the 3 finger swipes get handle!!
            else if (fingerCount == 3)
			{
				if (dir == ESDirection.EUp)
				{
					if (HandleUI())
					{
                        if (m_OnThreeFingerSwipeUpHandler != null)
                        {
                            m_OnThreeFingerSwipeUpHandler();
                        }
                        else
                        {
                            Debug.Log("CALL IS NULL");
				...
				}else if (dir == ESDirection.EDown)
                {
                    if (HandleUI())
                    {
                        if (m_OnThreeFingerSwipeDownHandler != null)
                        {
                            m_OnThreeFingerSwipeDownHandler();
                        }
                        else
                        {
                            Debug.Log("CALL IS NULL");
    ...
    /// <summary>
    /// Overrides the regular one finger swipe right function.
    /// Use this to handle the gesture yourself.
    /// This can be very useful during gameplay, if the gesture is used to read out stats or help
    /// instead.
    /// </summary>
    /// <param name="func">Member function to call when the gesture is detected.</param>
    static public void SetOneFingerSwipeRightHandler(OnTapEvent func)
    {
        Initialize();
        instance.m_OnOneFingerSwipeRightHandler = func;
    }

    static public void ResetOneFingerSwipeRightHandler()
    {
        Initialize();

        instance.m_OnOneFingerSwipeRightHandler = null;
    }
    ...
    /// <summary>
    /// Overrides the regular one finger swipe left function.
    /// Use this to handle the gesture yourself.
    /// This can be very useful during gameplay, if the gesture is used to read out stats or help
    /// instead.
    /// </summary>
    /// <param name="func">Member function to call when the gesture is detected.</param>
    static public void SetOneFingerSwipeLeftHandler(OnTapEvent func)
    {
        Initialize();
        instance.m_OnOneFingerSwipeLeftHandler = func;
    }

    static public void ResetOneFingerSwipeLeftHandler()
    {
        Initialize();

        instance.m_OnOneFingerSwipeLeftHandler = null;
    }

    //////////////////////////////////////////////////////////////////////////

    /// <summary>
    /// Overrides the regular one finger swipe UP function.
    /// Use this to handle the gesture yourself.
    /// This can be very useful during gameplay, if the gesture is used to read out stats or help
    /// instead.
    /// </summary>
    /// <param name="func">Member function to call when the gesture is detected.</param>
    static public void SetOneFingerSwipeUpHandler(OnTapEvent func)
    {
        Initialize();
        instance.m_OnOneFingerSwipeUpHandler = func;
    }

    static public void ResetOneFingerSwipeUpHandler()
    {
        Initialize();

        instance.m_OnOneFingerSwipeUpHandler = null;
    }

    //////////////////////////////////////////////////////////////////////////

    /// <summary>
    /// Overrides the regular one finger swipe Down function.
    /// Use this to handle the gesture yourself.
    /// This can be very useful during gameplay, if the gesture is used to read out stats or help
    /// instead.
    /// </summary>
    /// <param name="func">Member function to call when the gesture is detected.</param>
    static public void SetOneFingerSwipeDownHandler(OnTapEvent func)
    {
        Initialize();
        instance.m_OnOneFingerSwipeDownHandler = func;
        
    }

    static public void ResetOneFingerSwipeDownHandler()
    {
        Initialize();
        
        instance.m_OnOneFingerSwipeDownHandler = null;
    }

    //////////////////////////////////////////////////////////////////////////

    /// <summary>
    /// Overrides the regular double tap function, which Interacts with a selectable object.
    /// Use this to handle the gesture yourself. This will stop the 
    /// Interact with selectable object ::ResetDoubleTapHandler is called.<br><br>
    /// This can be very useful during gameplay, if the gesture is used to read out stats or help
    /// instead.
    /// </summary>
    /// <param name="func">Member function to call when the gesture is detected.</param>
    static public void SetDoubleTapHandler(OnTapEvent func)
    {
        Initialize();

        instance.m_OnDoubleTapHandler = func;
    }

    static public void ResetDoubleTapHandler()
    {
        Initialize();

        instance.m_OnDoubleTapHandler = null;
    }

    //////////////////////////////////////////////////////////////////////////

    /// <summary>
    /// Overrides the regular Long Press function, which Reads out the menu you're in.
    /// Use this to handle the gesture yourself. This will stop the 
    /// Interact with selectable object ::ResetLongPressHandler is called.<br><br>
    /// This can be very useful during gameplay, if the gesture is used to read out stats or help
    /// instead.
    /// </summary>
    /// <param name="func">Member function to call when the gesture is detected.</param>
    static public void SetLongPressHandler(OnTapEvent func)
    {
        Initialize();

        instance.m_OnLongPressHandler = func;
    }

    static public void ResetLongPressHandler()
    {
        Initialize();

        instance.m_OnLongPressHandler = null;
    }
    ...
    /// <summary>
    /// Overrides the regular two finger swipe right function.
    /// Use this to handle the gesture yourself. 
    /// This can be very useful during gameplay, if the gesture is used to read out stats or help
    /// instead.
    /// </summary>
    /// <param name="func">Member function to call when the gesture is detected.</param>
    static public void SetTwoFingerSwipeRightHandler(OnTapEvent func)
    {
        Initialize();
        instance.m_OnTwoFingerSwipeRightHandler = func;
    }

    static public void ResetTwoFingerSwipeRightHandler()
    {
        Initialize();
        instance.m_OnTwoFingerSwipeRightHandler = null;
    }
           
    //////////////////////////////////////////////////////////////////////////

    /// <summary>
    /// Overrides the regular two finger swipe left function.
    /// Use this to handle the gesture yourself.
    /// This can be very useful during gameplay, if the gesture is used to read out stats or help
    /// instead.
    /// </summary>
    /// <param name="func">Member function to call when the gesture is detected.</param>
    static public void SetTwoFingerSwipeLeftHandler(OnTapEvent func)
    {
        Initialize();
        instance.m_OnTwoFingerSwipeLeftHandler = func;
    }

    static public void ResetTwoFingerSwipeLeftHandler()
    {
        Initialize();

        instance.m_OnTwoFingerSwipeLeftHandler = null;
    }
    ...
    /// </summary>
    /// <param name="func">Member function to call when the gesture is detected.</param>
    static public void SetThreeFingerSwipeUpHandler(OnTapEvent func)
    {
        Initialize();

        instance.m_OnThreeFingerSwipeUpHandler = func;
    }

    static public void ResetThreeFingerSwipeUpHandler()
    {
        Initialize();

        instance.m_OnThreeFingerSwipeUpHandler = null;
    }
    ...
    /// </summary>
    /// <param name="func">Member function to call when the gesture is detected.</param>
    static public void SetThreeFingerSwipeDownHandler(OnTapEvent func)
    {
        Initialize();

        instance.m_OnThreeFingerSwipeDownHandler = func;
    }

    static public void ResetThreeFingerSwipeDownHandler()
    {
        Initialize();

        instance.m_OnThreeFingerSwipeDownHandler = null;
    }
